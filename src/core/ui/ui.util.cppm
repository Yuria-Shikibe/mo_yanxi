...
namespace mo_yanxi::ui::util{
	...
	constexpr bool try_modify(T& target, T&& value) noexcept(noexcept(target != value) && std::is_nothrow_move_assignable_v<T>){
		if(target != value){
	...
	constexpr bool try_modify(T& target, const T& value) noexcept(noexcept(target != value) && std::is_nothrow_copy_assignable_v<T>){
		if(target != value){
			target = value;
			return true;
		}
