...
struct std::hash<mo_yanxi::graphic::render_graph::pass_identity>{
	static std::size_t operator()(const mo_yanxi::graphic::render_graph::pass_identity& idt) noexcept{
		if(idt.where){
			...
			auto p1 = std::bit_cast<std::size_t>(idt.external_target) ^ static_cast<std::size_t>(idt.slot_out) ^ (
				static_cast<std::size_t>(idt.slot_in) << 31);
...
protected:
	...
	virtual void reset_resources(vk::context& context, const pass_resource_reference& resources,
	...
	void unique(){
		...
		while(itr != end){
			...
			if(indices != nullptr){
				...
				assert(indices->requirement.must_compatible_with(itr->requirement));
				itr->requirement.promote(indices->requirement);
...
public:
	...
	external_descriptor& add_external_descriptor(external_descriptor desc){
...
private:
	...
	void check_sockets_connection() const{
		for(const auto& stage : execute_sequence_ | ranges::views::deref){
			...
			std::unordered_map<inout_index, const resource_desc::resource_requirement*> resources{
					inout.input_slots.size()
				};
	...
	void analysis_minimal_allocation(){
		...
		struct assignment{
			std::uint32_t partition_index{no_slot};
			std::uint32_t res_index{no_slot};
		};

		std::vector<assignment> lifebound_assignments(life_bounds.size());
		...
		auto get_partition_itr = [&](const resource_requirement& r){
			return std::ranges::find_if(minimal_requirements_per_partition, [&](const partition& req){
				return req.identity().must_compatible_with(r);
			});
		};
...

		{
			for(auto&& [idx, image] : life_bounds | std::views::enumerate){
				...
				const auto category = get_partition_itr(image.requirement);
				lifebound_assignments[idx].partition_index = category - minimal_requirements_per_partition.begin();

				if(category == minimal_requirements_per_partition.end()){
			...
			for(auto&& [bound, assignment] : std::views::zip(life_bounds, lifebound_assignments)){
				...
				if(assignment.res_index != no_slot) continue;

				auto& partition_itr = minimal_requirements_per_partition[assignment.partition_index];
				...
				for(auto& candidate : partition_itr.indices){
					...
					if(comp(bound.requirement, partition_itr.requirements[candidate])){
						...
						assignment.res_index = candidate;
				...
				assignment.res_index = partition_itr.add(bound.requirement);
				for(auto passed_by : subrange){
					partition_itr.stage_captures.at(passed_by)[assignment.res_index] = true;
				}
		...
		for(auto&& [bound, assignment] : std::views::zip(life_bounds, lifebound_assignments)){
			if(bound.ownership != ownership_type::shared) continue;
			assert(assignment.res_index != no_slot);

			auto& pt = minimal_requirements_per_partition[assignment.partition_index];
			bound.used_entity = &shared_resources[pt.prefix_sum + assignment.res_index];
...
std::string pass_identity::format(std::string_view endpoint_name) const{
	...
	return std::format("[{}] {} [{}]", fmt_slot(endpoint_name, slot_in),
	                   std::string(where ? where->get_identity_name() : "endpoint"), fmt_slot(endpoint_name, slot_out));
}
}
