...
export
template <>
struct key_binding<void> : key_set{
	...
	template <typename... ParamTy>
	FORCE_INLINE binding_return cast_and_exec(float press_dur, ParamTy... args) const;
};

template <typename ... ParamTy>
key_binding<void>::operator key_binding<ParamTy...>() const noexcept{
	return std::bit_cast<key_binding<ParamTy...>>(*this);
}

template <typename ... ParamTy>
binding_return key_binding<void>::cast_and_exec(float press_dur, ParamTy... args) const{
	CHECKED_ASSUME(func != nullptr);
	return static_cast<typename key_binding<ParamTy...>::function_ptr>(func)(static_cast<key_set>(*this), press_dur, std::forward<ParamTy>(args)...);
}
...
struct key_binding : key_set{
	...
	friend constexpr bool operator==(const key_binding& lhs, const key_binding<void>& rhs){
		return key_binding<void>(lhs) == rhs;
	}

	friend constexpr bool operator==(const key_binding<void>& lhs, const key_binding& rhs){
		return lhs == key_binding<void>(rhs);
	}

	explicit(false) operator key_binding<void>() const noexcept{
		return {static_cast<key_set>(*this), func};
	}
};
			...
			for (const auto & binding : bindings){
				if(matched(action, binding.action) && matched(mode_bits, binding.mode_bits)){
					const binding_return rst = [&, this]<std::size_t ...Idx>(std::index_sequence<Idx...>){
						return binding.cast_and_exec<ParamTy ...>(press_duration, std::get<Idx>(context)...);
...
protected:
	bool add_raw(key_binding<void> erased_binding){
...
public:
	bool erase_binding(key_binding<void> erased_binding) noexcept{
		const auto idx = erased_binding.key_code;
		auto where = std::ranges::lower_bound(keys_, idx, {}, &key_index::key_code);
...
public:
	...
	bool add_binding(key_set key, bind_type_with_args::function_ptr fn){
		return add_raw(bind_type{key, fn});
	}
	template <typename Fn>
