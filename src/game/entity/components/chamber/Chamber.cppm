...
namespace mo_yanxi::game::ecs::chamber{
	export constexpr int TileSize_Integral = 25;
	export constexpr float TileSize = TileSize_Integral;
	using IdType = unsigned;

	export
	template <typename Ety>
	struct Building : ext::referenced_object<false>, Geom::QuadTreeAdaptable<Building<Ety>>{
	protected:
		constexpr ~Building() = default;
	public:
		using EntityType = Ety;
		Property prop{};

		[[nodiscard]] constexpr Geom::Point2 pos() const noexcept{
			return prop.bound.src;
		}

		[[nodiscard]] constexpr Geom::Rect_Orthogonal<float> getBound() const noexcept{
			return prop.getEntityBound();
		}

		[[nodiscard]] bool roughIntersectWith(const Building& other) const = delete;

		[[nodiscard]] bool exactIntersectWith(const Building& other) const = delete;

		[[nodiscard]] bool containsPoint(Geom::Vec2 point) const noexcept{
	...
	struct building_property{
		...
		[[nodiscard]] constexpr Geom::OrthoRectFloat getEntityBound() const noexcept{
			return bound.as<float>().scl(TileSize, TileSize);
		}
	};

	export
	template <typename Ety, std::derived_from<Building<Ety>> B = Building<Ety>>
	struct EmptyBuilding final : B{
		[[nodiscard]] constexpr EmptyBuilding() = default;
		[[nodiscard]] explicit constexpr EmptyBuilding(Geom::OrthoRectInt bound){
			this->prop.bound = bound;
		}
		//
		// void update(float delta, typename B::EntityType& entity) override{}
		//
		// void draw(const typename B::EntityType& entity) const override{}
	};

	template <typename Ety>
	constexpr EmptyBuilding<Ety> EmptyBuilding{};

	export
	template <typename Ety, typename SpecBuildType = Building<Ety>>
	struct Tile{
		using EntityType = Ety;
		using BuildType = SpecBuildType;

		ext::referenced_ptr<BuildType> build{};

		Geom::Point2 pos{};
		bool placeable{};

		void resetBuild() noexcept{
			build.reset();
		}

		[[nodiscard]] bool isBuildIdentical() const noexcept requires(std::derived_from<BuildType, Building<Ety>>){
			return build && build->pos() == pos;
		}

		Tile& operator<<(const ext::dim2::vec<int> val) noexcept{
			pos = val;
			return *this;
		}
		...
		[[nodiscard]] constexpr Geom::OrthoRectFloat getBound() const noexcept{
			return Geom::OrthoRectInt{Geom::FromUnchecked, pos.x, pos.y, 1, 1}.as<float>().scl(TileSize, TileSize);
		}

		[[nodiscard]] constexpr Geom::Vec2 getRealPos() const noexcept{
			return pos.as<float>() * TileSize + Geom::Vec2{TileSize, TileSize} / 2.f;
		}
		[[nodiscard]] constexpr Geom::Point2 getLocalPos() const noexcept requires(std::derived_from<BuildType, Building<Ety>>){
			if(build){
				return pos - build->pos();
			}
			return {};
		}
	};
	...
	//
	// namespace Traits{
	// 	template <typename Ety, std::derived_from<ChamberBuild<Ety>> TraitBuild, std::derived_from<ChamberData> TraitData>
	// 	struct ChamberBuildTrait{
	// 		using EntityType = Ety;
	// 		using BuildType = TraitBuild;
	// 		using DataType = TraitData;
	//
	// 		void update(BuildType&, EntityType&, const float) const{}
	// 		//
