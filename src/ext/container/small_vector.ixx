      ...
      { };
        ...
        std::abort ();
...
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
				if(std::is_constant_evaluated()) return default_uninitialized_copy(first, last, dest);
#endif

				const size_ty num_copy = external_range_length(first, last);
				if(num_copy != 0) std::memcpy(to_address(dest), to_address(first), num_copy * sizeof(value_ty));
			...
			default_uninitialized_copy(InputIt first, InputIt last, ptr d_first){
				...
				GCH_TRY{
					...
					for(; !(first == last); ++first, static_cast<void>(++d_last)) construct(d_last, *first);
					return d_last;
				}
				GCH_CATCH(...){
...
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
				if(std::is_constant_evaluated()) return default_uninitialized_value_construct(first, last);
#endif
		...
		private:
			...
			construct_at(value_ty* p, Args&&... args)
				...
				if(std::is_constant_evaluated()) return std::construct_at(p, std::forward<Args>(args)...);
#endif
		...
		protected:
			...
			move_assign_default(small_vector_base<Allocator, LessEqualI>&& other)
				...
				if(InlineCapacity < other.get_capacity()) move_allocation_pointer(std::move(other));
				else{
					// We are guaranteed to have sufficient capacity to store the elements.
					if(InlineCapacity < get_capacity()){
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
						ptr new_data_ptr;
						if(std::is_constant_evaluated()) new_data_ptr = other.allocate(InlineCapacity);
						else new_data_ptr = storage_ptr();
#else
            const ptr new_data_ptr = storage_ptr ();
#endif
			...
			move_assign_default(small_vector_base<Allocator, GreaterI>&& other){
				if(other.has_allocation()) move_allocation_pointer(std::move(other));
			...
			small_vector_base&
			move_assign(small_vector_base<Allocator, I>&& other){
				if(other.allocator_ref() == allocator_ref()) return move_assign_default(std::move(other));
      ...
      template <typename ForwardIt>
		...
		protected:
			...
			set_to_inline_storage(void){
				set_capacity(InlineCapacity);
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
				if(std::is_constant_evaluated()) return set_data_ptr(alloc_interface::allocate(InlineCapacity));
#endif
			...
			assign_with_copies(size_ty count, const value_ty& val){
				if(get_capacity() < count){
					...
					set_size(count);
				} else erase_range(std::fill_n(begin_ptr(), count, val), end_ptr());
			}
			...
			assign_with_range(InputIt first, InputIt last, std::input_iterator_tag){
				...
				ptr curr = begin_ptr();
				for(; !(end_ptr() == curr || first == last); ++curr, static_cast<void>(++first)) *curr = *first;

				if(first == last) erase_to_end(curr);
				else append_range(first, last, iterator_cat{});
			}
			...
			assign_with_range(ForwardIt first, ForwardIt last, std::forward_iterator_tag){
				...
				if(get_capacity() < count){
					...
					set_size(count);
				} else erase_range(copy_range(first, last, begin_ptr()), end_ptr());
			}
			...
			append_element(Args&&... args){
				if(get_size() < get_capacity()) return emplace_into_current_end(std::forward<Args>(args)...);
			...
			emplace_at(ptr pos, Args&&... args){
				...
				if(get_size() < get_capacity()) return emplace_into_current(pos, std::forward<Args>(args)...);
			...
			emplace_into_reallocation(ptr pos, Args&&... args){
				const size_ty offset = internal_range_length(begin_ptr(), pos);
				if(offset == get_size()) return emplace_into_reallocation_end(std::forward<Args>(args)...);
...
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
					if(std::is_constant_evaluated()) new_data_ptr = alloc_interface::allocate(InlineCapacity);
					else new_data_ptr = storage_ptr();
#else
          new_data_ptr = storage_ptr ();
			...
			resize_with(size_ty new_size, const ValueT&... val){
				...
				if(get_capacity() < new_size){
					...
					set_size(new_size);
				} else erase_range(unchecked_next(begin_ptr(), new_size), end_ptr());
			...
			swap_default(small_vector_base& other)
              ...
              &&  detail::small_vector_adl::is_nothrow_swappable<value_ty>::value
				...
				if(has_allocation()) // Implies that `other` also has an allocation.
					...
					swap_size(other);
				} else if(get_size() < other.get_size()) swap_elements(other);
				else other.swap_elements(*this);
			...
			swap(small_vector_base& other)
              ...
              &&  detail::small_vector_adl::is_nothrow_swappable<value_ty>::value
				...
				if(get_capacity() < other.get_capacity()) swap_default(other);
				else other.swap_default(*this);
			}
			...
			swap(small_vector_base& other){
				if(get_capacity() < other.get_capacity()){
					if(other.allocator_ref() == allocator_ref()) swap_default(other);
					else swap_unequal_no_propagate(other);
				} else{
					if(other.allocator_ref() == allocator_ref()) other.swap_default(*this);
					else other.swap_unequal_no_propagate(*this);
				}
			}
        ...
        if GCH_IF_CONSTEXPR (! std::is_same<decltype (unmove_iterator (first)), InputIt>::value)
          if (std::is_constant_evaluated ())
			...
			copy_n_return_in(InputIt first, size_ty count, ptr dest) noexcept{
				...
				if(count != 0) std::memcpy(to_address(dest), to_address(first), count * sizeof(value_ty));
        ...
        if GCH_IF_CONSTEXPR (! std::is_same<decltype (unmove_iterator (first)), RandomIt>::value)
        {
          if (std::is_constant_evaluated ())
          {
			...
			copy_n_return_in(InputIt first, size_ty count, ptr dest){
				for(; count != 0; --count, static_cast<void>(++dest), static_cast<void>(++first)) *dest = *first;
				return first;
			}
			...
			move_left(ptr first, ptr last, ptr d_first){
				...
				if(num_moved != 0) std::memmove(to_address(d_first), to_address(first), num_moved * sizeof(value_ty));
			...
			move_right(ptr first, ptr last, ptr d_last){
				...
				if(num_moved != 0) std::memmove(to_address(dest), to_address(first), num_moved * sizeof(value_ty));
				return dest;
			}
