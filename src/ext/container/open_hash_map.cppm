...
namespace mo_yanxi{
	//TODO remove the nonsense reinterpret cast
		...
		struct hm_iterator_dynamic{
			using container_type = std::conditional_t<addConst, const open_addr_hash_map_base, open_addr_hash_map_base>;
			using difference_type = std::ptrdiff_t;
			using value_type = std::conditional_t<addConst, const value_type_external, value_type_external>;
			using mapped_type = std::conditional_t<addConst, const mapped_type, mapped_type>;
			...
			[[nodiscard]] hm_iterator_dynamic() = default;

			constexpr bool operator==(const hm_iterator_dynamic& other) const noexcept = default;

			constexpr hm_iterator_dynamic& operator++(){
				++idx_;
				advance_past_empty();
				return *this;
			}

			constexpr hm_iterator_dynamic operator++(int){
				auto itr = *this;
				++(*this);
				return itr;
			}
			...
			constexpr explicit hm_iterator_dynamic(container_type* hm) noexcept : hm_(hm){ advance_past_empty(); }

			constexpr hm_iterator_dynamic(container_type* hm, const size_type idx) noexcept : hm_(hm), idx_(idx){}
		...
		private:
			...
			explicit(false) hm_iterator_dynamic(const hm_iterator_dynamic<oAddConst>& other)
				: hm_(other.hm_), idx_(other.idx_){}

			constexpr void advance_past_empty() noexcept{
				while(idx_ < hm_->buckets_.size() &&
					KeyEqualer(hm_->buckets_[idx_].first, hm_->empty_key())){
					++idx_;
				}
			}

			[[nodiscard]] constexpr std::size_t getIdx(auto) const noexcept{
				return idx_;
			}
	...
	// using EmptyKey = int;
	// constexpr EmptyKey emptyKey = 0;
	// using Convertor = std::identity;
