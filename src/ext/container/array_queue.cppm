...
export namespace mo_yanxi {
    ...
    public:
        ...
        constexpr void push_back(T&& item) noexcept(!DEBUG_CHECK && std::is_nothrow_move_assignable_v<T>)
        ...
        constexpr T& emplace_back(Args&&... args) noexcept(!DEBUG_CHECK && noexcept(T{ std::forward<Args>(args)... }))
        {
#if DEBUG_CHECK
            if (full()) {
                throw std::overflow_error("Queue is full");
        ...
        constexpr void push_front(const T& item) noexcept(!DEBUG_CHECK && std::is_nothrow_copy_assignable_v<T>)
            requires (std::is_copy_assignable_v<T>)
        {
#if DEBUG_CHECK
            if (full()) {
        ...
        constexpr void push_front(T&& item) noexcept(!DEBUG_CHECK && std::is_nothrow_move_assignable_v<T>)
            requires (std::is_move_assignable_v<T>)
        {
#if DEBUG_CHECK
            if (full()) {
        ...
        template <typename... Args>
            requires (std::constructible_from<T, Args...>)
        constexpr T& emplace_front(Args&&... args) noexcept(!DEBUG_CHECK && noexcept(T{ std::forward<Args>(args)... }))
        {
#if DEBUG_CHECK
            if (full()) {
                throw std::overflow_error("Queue is full");
        ...
        constexpr void push_back_and_replace(T&& element) noexcept(!DEBUG_CHECK && std::is_nothrow_move_assignable_v<T>) {
            if (full()) {
                pop_front();
            }
        ...
        template <typename... Args>
        constexpr decltype(auto) emplace_back_and_replace(Args&&... args) noexcept(!DEBUG_CHECK && noexcept(T{ std::forward<Args>(args)... })) {
            if (full()) {
                pop_front();
            }
