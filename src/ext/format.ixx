...
namespace mo_yanxi{
	...
	struct static_str{
		...
		[[nodiscard]] constexpr static_str() = default;

		[[nodiscard]] constexpr explicit(false) static_str(const value_type (&str)[Sz]) noexcept{
...
struct std::formatter<mo_yanxi::static_str<CharT, Sz>>{
	static constexpr auto parse(std::basic_format_parse_context<CharT>& context){
		return context.begin();
	}

	template <typename OutputIt>
	OutputIt format(const mo_yanxi::static_str<CharT, Sz>& c, std::basic_format_context<OutputIt, CharT>& context) const{
...
namespace mo_yanxi{
	...
	metric_prefix_formatter<T, 3, static_str{"K"}, static_str{"M"}, static_str{"B"}, static_str{"T"}, static_str{"Qa"}, static_str{"Qi"}> {
		return {arth};
	}
}
